...
void sim_report(void)
{
	...
	fprintf(SETUP_REPORT_FILE, "clock,kW,total_kW,kW_npg,total_kW_npg,node,rack,total_resp_t,resp_t_count,avg_resp_t,total_q_delay_t,q_delay_count,avg_q_delay,local_node,local_rack,local_remote,map_tasks,task_t,cpu_t,mem_t,disk_t,net_t,task_q_t,avg_m,active_cap,m/cap,req_m,req_node\n");
	...
	while (!CSIM_END) {
		...
		REPORT_RACK.first += ACTIVE_RACK_SET.size();
		REPORT_RACK.second++;
		...
		if (t > 0 && (t % hold_t) == 0) {
			...
			fprintf(SETUP_REPORT_FILE, "%ld,%lf,%lf,%lf,%lf,%ld,%ld,%lf,%ld,%lf,%lf,%ld,%lf,%ld,%ld,%ld,%ld,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%ld,%lf,%ld,%ld\n",
				...
				REPORT_REQ_M.second > 0 ? (REPORT_REQ_M.first / REPORT_REQ_M.second) : 0, 
