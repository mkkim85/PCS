...
process_class* process_process_class(process_t ptr);
...
public:
	...
	process_class* this_process_class() {return(process_class*)get_process_class((PROCESS_PTR)this);}
...
public:
	...
	void server_stats_on() {::server_stats_on(f);}
	void server_stats_off(){::server_stats_off(f);}
	...
	double server_serv_time_remaining(long n) {return(::server_serv_time_remaining(f, n));}
...
public:
	...
	void collect_class_storage()		{::collect_class_storage(s);}
	...
	double class_busy_amt(process_class_t c) {return storage_class_busy_amt(s, c->ptr());}
	double class_waiting_amt(process_class_t c) {return storage_class_waiting_amt(s, c->ptr());}
	long class_request_cnt(process_class_t c) {return storage_class_request_cnt(s, c->ptr());}
	long class_release_cnt(process_class_t c) {return storage_class_release_cnt(s, c->ptr());}
	double class_request_total(process_class_t c) {return storage_class_request_total(s, c->ptr());}
	double class_release_total(process_class_t c) {return storage_class_release_total(s, c->ptr());}
	double class_request_amt(process_class_t c)	{return storage_class_request_amt(s, c->ptr());}
...
public:
	...
	void collect_class_buffer()	{::collect_class_buffer(buf);}
	...
	long class_get_total(process_class_t c) {return buffer_class_get_total(buf, c->ptr());}
	long class_put_total(process_class_t c)	{return buffer_class_put_total(buf, c->ptr());}
	long class_get_count(process_class_t c)	{return buffer_class_get_count(buf, c->ptr());}
	long class_put_count(process_class_t c)	{return buffer_class_put_count(buf, c->ptr());}
	double class_get_timeQueue(process_class_t c){return buffer_class_get_timeQueue(buf, c->ptr());}
	double class_put_timeQueue(process_class_t c){return buffer_class_put_timeQueue(buf, c->ptr());}
...
public:
   ...
   void compute_confidence_statistics (double conf_level,
	   double *mean_of_means, double *half_width, double *rel_error)
		{::compute_confidence_statistics(t , conf_level, mean_of_means, half_width, rel_error);}
...
public:
	...
	double erlang(double u, double v) { return(::stream_erlang((STREAM)s, u, v)); }
	double expntl(double x)		{ return(::stream_expntl((STREAM)s, x)); }
	double hyperx(double u, double v) { return(::stream_hyperx((STREAM)s, u, v)); }
#ifndef MAC
#ifndef MAC_PPC
	double normal(double u, double std) { return(::stream_normal((STREAM)s, u, std)); }
#else
	double x_normal(double u, double std) { return(::stream_normal((STREAM)s, u, std)); }
#endif
#else
	double x_normal(double u, double std) { return(::stream_normal((STREAM)s, u, std)); }
#endif
	long random(long i1, long i2)	{ return(::stream_uniform_int((STREAM)s, i1, i2)); }
	long random_int(long i1, long i2) { return(::stream_uniform_int((STREAM)s, i1, i2)); }
	double prob()			{ return(::stream_prob((STREAM)s)); }
	void reset_prob(long n)	{ ::stream_reset_prob1(s1, n); }
	double uniform(double x1, double x2) {return(::stream_uniform((STREAM)s, x1, x2)); }
	
	/* JAB additions */
	
	double uniform01 ( )     { return (::stream_uniform01 ((STREAM)s)); }
	double triangular (double minimum, double maximum, double mode)
	                         { return (::stream_triangular ((STREAM)s, minimum, maximum, mode)); }
	double gamma (double mean, double stddev)
	                         { return (::stream_gamma ((STREAM)s, mean, stddev)); }
	double exponential (double mean)
	                         { return (::stream_exponential ((STREAM)s, mean)); }
	double erlang2 (double mean, double var)
	                         { return (::stream_erlang2 ((STREAM)s, mean, var)); }
	double erlangn (double mean, long n)
							{return (::stream_erlangn((STREAM)s, mean, n)); }
	double beta (double minimum, double maximum, double shape1, double shape2)
	                         { return (::stream_beta ((STREAM)s, minimum, maximum, shape1, shape2)); }
	double weibull (double shape, double scale)
	                         { return (::stream_weibull ((STREAM)s, shape, scale)); }
	double normal2 (double mean, double stddev)
	                         { return (::stream_normal2 ((STREAM)s, mean, stddev)); }
	double lognormal (double mean, double stddev)
	                         { return (::stream_lognormal ((STREAM)s, mean, stddev)); }
	double cauchy (double alpha, double beta)
	                         { return (::stream_cauchy ((STREAM)s, alpha, beta)); }
	double hypoexponential (double mn, double var)
							{return (::stream_hypoexponential ((STREAM)s, mn, var)); }
	double pareto(double a)
							{return (::stream_pareto((STREAM)s, a));}
	long zipf(long n)
							{return (::stream_zipf((STREAM)s, n)); }
	long zipf_sum(long n, double *sum)
							{return (::stream_zipf_sum((STREAM)s, n, sum)); }
	long uniform_int (long minimum, long maximum)
	                         { return (::stream_uniform_int ((STREAM)s, minimum, maximum)); }
	long bernoulli (double probsuccess)
	                         { return (::stream_bernoulli ((STREAM)s, probsuccess)); }
	long binomial (double probsuccess, long numtrials)
	                         { return (::stream_binomial ((STREAM)s, probsuccess, numtrials)); }
	long geometric (double probsuccess)
	                         { return (::stream_geometric ((STREAM)s, probsuccess)); }
	long negative_binomial (long successnum, double probsuccess)
	                         { return (::stream_negative_binomial ((STREAM)s, successnum, probsuccess)); }
	long poisson (double mean)
	                         { return (::stream_poisson ((STREAM)s, mean)); }
	double empirical (const long n, double cutoff[], long alias[], const double value[])
                             { return (::stream_empirical ((STREAM)s, n, cutoff, alias, value)); }
};
...
protected:
	STREAM2 sx;
public:
	csim_stream2()	{sx = create_stream2(); s = sx; s1 = NIL; s2 = sx; set_stream_prob2(stream_prob2);}
};
